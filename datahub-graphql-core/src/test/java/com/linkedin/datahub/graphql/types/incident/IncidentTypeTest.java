...
public class IncidentTypeTest {
  ...
  private static final IncidentKey TEST_INCIDENT_KEY = new IncidentKey()
      .setId("guid-1");
  private static final IncidentInfo TEST_INCIDENT_INFO = new IncidentInfo()
      ...
      .setSource(new IncidentSource().setType(IncidentSourceType.ASSERTION_FAILURE).setSourceUrn(testAssertionUrn))
      .setStatus(new IncidentStatus().setState(IncidentState.ACTIVE).setMessage("Message").setLastUpdated(
          new AuditStamp().setTime(1L).setActor(testUserUrn)
      ))
  ...
  public void testBatchLoad() throws Exception {
    ...
    com.linkedin.datahub.graphql.types.incident.IncidentType type = new com.linkedin.datahub.graphql.types.incident.IncidentType(client);

    QueryContext mockContext = Mockito.mock(QueryContext.class);
    Mockito.when(mockContext.getAuthentication()).thenReturn(Mockito.mock(Authentication.class));
    List<DataFetcherResult<Incident>> result = type.batchLoad(ImmutableList.of(TEST_INCIDENT_URN, TEST_INCIDENT_URN_2), mockContext);
    ...
    assertEquals(incident.getStatus().getState().toString(), TEST_INCIDENT_INFO.getStatus().getState().toString());
    assertEquals(incident.getStatus().getMessage(), TEST_INCIDENT_INFO.getStatus().getMessage());
    assertEquals(incident.getStatus().getLastUpdated().getTime(), TEST_INCIDENT_INFO.getStatus().getLastUpdated().getTime());
    assertEquals(incident.getStatus().getLastUpdated().getActor(), TEST_INCIDENT_INFO.getStatus().getLastUpdated().getActor().toString());
    assertEquals(incident.getSource().getType().toString(), TEST_INCIDENT_INFO.getSource().getType().toString());
    assertEquals(incident.getSource().getSource().getUrn(), TEST_INCIDENT_INFO.getSource().getSourceUrn().toString());
    assertEquals(incident.getCreated().getActor(), TEST_INCIDENT_INFO.getCreated().getActor().toString());
  ...
  public void testBatchLoadClientException() throws Exception {
    ...
    Mockito.doThrow(RemoteInvocationException.class).when(mockClient).batchGetV2(
        ...
        Mockito.anySet(),
        Mockito.any(Authentication.class));
    com.linkedin.datahub.graphql.types.incident.IncidentType type = new com.linkedin.datahub.graphql.types.incident.IncidentType(mockClient);
    ...
    assertThrows(RuntimeException.class, () -> type.batchLoad(ImmutableList.of(TEST_INCIDENT_URN, TEST_INCIDENT_URN_2),
        context));
  }
}