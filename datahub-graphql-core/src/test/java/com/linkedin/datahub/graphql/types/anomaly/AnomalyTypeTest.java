...
public class AnomalyTypeTest {
  ...
  private static final AnomalyKey TEST_ANOMALY_KEY = new AnomalyKey()
      .setId("guid-1");

  private static final AnomalyInfo TEST_ANOMALY_INFO = new AnomalyInfo()
  ...
  public void testBatchLoad() throws Exception {
    ...
    com.linkedin.datahub.graphql.types.anomaly.AnomalyType type = new com.linkedin.datahub.graphql.types.anomaly.AnomalyType(client);

    QueryContext mockContext = Mockito.mock(QueryContext.class);
    Mockito.when(mockContext.getAuthentication()).thenReturn(Mockito.mock(Authentication.class));
    List<DataFetcherResult<Anomaly>> result = type.batchLoad(ImmutableList.of(
        TEST_ANOMALY_URN.toString(),
        TEST_ANOMALY_URN_2.toString()), mockContext);
    ...
    assertEquals(anomaly.getCreated().getActor(), TEST_ANOMALY_INFO.getCreated().getActor().toString());
    assertEquals(anomaly.getEntity().getUrn(), TEST_ANOMALY_INFO.getEntity().toString());
    assertEquals(anomaly.getStatus().getState().toString(), TEST_ANOMALY_INFO.getStatus().getState().toString());
    assertEquals(anomaly.getStatus().getLastUpdated().getTime(), TEST_ANOMALY_INFO.getStatus().getLastUpdated().getTime());
    assertEquals(anomaly.getStatus().getLastUpdated().getActor(), TEST_ANOMALY_INFO.getStatus().getLastUpdated().getActor().toString());
    assertEquals(anomaly.getSource().getType().toString(), TEST_ANOMALY_INFO.getSource().getType().toString());
    assertEquals(anomaly.getReview().getState().toString(), TEST_ANOMALY_INFO.getReview().getState().toString());
    assertEquals(anomaly.getReview().getLastUpdated().getTime(), TEST_ANOMALY_INFO.getReview().getLastUpdated().getTime());
    assertEquals(anomaly.getReview().getLastUpdated().getActor(), TEST_ANOMALY_INFO.getReview().getLastUpdated().getActor().toString());
  ...
  public void testBatchLoadClientException() throws Exception {
    ...
    Mockito.doThrow(RemoteInvocationException.class).when(mockClient).batchGetV2(
        ...
        Mockito.anySet(),
        Mockito.any(Authentication.class));
    com.linkedin.datahub.graphql.types.anomaly.AnomalyType type = new com.linkedin.datahub.graphql.types.anomaly.AnomalyType(mockClient);
