...
public class UpdateSystemMonitorsResolver implements DataFetcher<CompletableFuture<Boolean>> {
  ...
  @Override
  public CompletableFuture<Boolean> get(@Nonnull final DataFetchingEnvironment environment) throws Exception {
    final QueryContext context = environment.getContext();
    final UpdateSystemMonitorsInput input = ResolverUtils.bindArgument(environment.getArgument("input"), UpdateSystemMonitorsInput.class);
      ...
      if (isAuthorizedToUpdateEntityMonitors(entityUrn, context)) {
        ...
        try {
          for (final UpdateSystemMonitorInput monitorInput : input.getMonitors()) {
            ...
            final Urn monitorUrn = getMonitorUrnForSystemMonitorType(entityUrn, monitorInput.getType());
