...
public class IntegrationsServiceTest {
  ...
  public void testGetLinkPreviewSuccess() throws Exception {
    ...
    String expectedJson = "{\"url\":\"https://test.slack.com/archives/something\", \"testField\": 10, \"lastRefreshedMs\": 1234567890}";
    ...
    assertEquals("localhost:8080/private/get_link_preview", request.getURI().getAuthority() + request.getURI().getPath());
    assertEquals(String.format("{\n  \"type\" : \"SLACK_MESSAGE\",\n  \"url\" : \"%s\"\n}", url),
      EntityUtils.toString(((StringEntity) request.getEntity())));
    assertEquals(LinkPreviewType.SLACK_MESSAGE, result.getType());
    assertEquals("{\"url\":\"https://test.slack.com/archives/something\",\"testField\":10,\"lastRefreshedMs\":1234567890}", result.getJson());
  ...
  public void testGetLinkPreviewException() throws Exception {
    ...
    String expectedJson = "{\"url\":\"https://test.slack.com/archives/something\", \"testField\": 10, \"lastRefreshedMs\": 1234567890}";
    CloseableHttpResponse mockResponse = mock(CloseableHttpResponse.class);
    when(mockResponse.getStatusLine()).thenReturn(new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 500, "ERROR"));
